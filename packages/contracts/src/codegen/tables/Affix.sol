...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("Affix")))
);
ResourceId constant AffixTableId = _tableId;
...
library Affix {
   ...
   * @notice Get partId.
   */
  function _getPartId(bytes32 entity) internal view returns (AffixPartId partId) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = entity;
   ...
   * @notice Set partId.
   */
  function _setPartId(bytes32 entity, AffixPartId partId) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = entity;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 entity) internal view returns (AffixData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = entity;
  ...
  function decodeStatic(
    ...
    partId = AffixPartId(uint8(Bytes.slice1(_blob, 0)));

    protoEntity = (Bytes.slice32(_blob, 1));

    value = (uint32(Bytes.slice4(_blob, 33)));
  }
