...
library UniqueIdx_SkillName_Name {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "skill", name: "SkillName", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462736b696c6c000000000000000000536b696c6c4e616d6500000000000000);
  ...
  function getKeyTuple(string memory name) internal view returns (bytes32[] memory _keyTuple) {
    ...
    if (_keyTuple.length == 0) {
      revert IIdxErrors.UniqueIdx_InvalidGet({
        tableId: _tableId,
        libraryName: "UniqueIdx_SkillName_Name",
