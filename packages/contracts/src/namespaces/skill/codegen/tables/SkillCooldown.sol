...
library SkillCooldown {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "skill", name: "SkillCooldown", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462736b696c6c000000000000000000536b696c6c436f6f6c646f776e000000);
  ...
  ) internal view returns (SkillCooldownData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = targetEntity;
    _keyTuple[1] = applicationEntity;
   ...
   */
  function set(bytes32 targetEntity, bytes32 applicationEntity, SkillCooldownData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 targetEntity, bytes32 applicationEntity, SkillCooldownData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timeId, _table.timeValue);
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (SkillCooldownData memory _table) {
