...
library UniqueIdx_SOFClassName_Name {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "common", name: "SOFClassName", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462636f6d6d6f6e0000000000000000534f46436c6173734e616d6500000000);
  ...
  function getKeyTuple(string memory name) internal view returns (bytes32[] memory _keyTuple) {
    ...
    if (_keyTuple.length == 0) {
      revert IIdxErrors.UniqueIdx_InvalidGet({
        tableId: _tableId,
        libraryName: "UniqueIdx_SOFClassName_Name",
