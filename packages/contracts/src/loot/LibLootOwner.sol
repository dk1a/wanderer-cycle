...
library LibLootOwner {
  ...
  function setSimpleOwnership(IUint256Component components, uint256 lootEntity, uint256 ownerEntity) internal {
  ...
  function setTradeableOwnership(IUint256Component systems, uint256 lootEntity, address ownerAccount) internal {
    WNFTSystem wnftSystem = WNFTSystem(getAddressById(systems, WNFTSystemID));
    wnftSystem.executeSafeMint(ownerAccount, lootEntity, "");
  }

  function ownerOf(IUint256Component components, uint256 lootEntity) internal view returns (uint256) {
